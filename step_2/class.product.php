...
class Product
{
	...
	public function create($name,$price,$category_id)
	{
		try
		{
			...
			$stmt->execute();
			return 'product sucressfully create!';
		}
		...
		public function dataview($query)
		{
			...
			if($stmt->rowCount()>0)
			{
				while($row=$stmt->fetch(PDO::FETCH_ASSOC))
				{
					...
					$nested->execute(array(':parent' => $row["category_id"]));
			...
			if($stmt->rowCount()>0) {
				while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
					...
					<tr>
						...
						<td><?php print($parent['name']); ?></td>
						<td align='center'>
							<a href='edit-product.php?edit_id=<?php print($row['id']); ?>'><i class='glyphicon glyphicon-edit'></i></a>
						</td>
						<td align='center'>
							<a href='delete-product.php?delete_id=<?php print($row['id']); ?>'><i class='glyphicon glyphicon-remove-circle'></i></a>
						</td>
					</tr>
					<?php
				}
